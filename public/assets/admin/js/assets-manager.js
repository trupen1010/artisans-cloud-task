$(document).ready(function () {
    if (typeof fileTypeMaxSize === 'undefined') {
        fileTypeMaxSize = {
            'image/jpg': 5,
            'image/jpeg': 5,
            'image/png': 5,
    ...
    // Initialize Each FilePond Instance Size.
    let totalUploadedSize = 0; // in bytes
    const totalUploadedLimitSize = remainingStorageSize; // GB to bytes
    ...
    inputMultipleElements.forEach(function (inputElement) {
        pond = FilePond.create(inputElement, {
            ...
            onaddfile: (error, file) => {
                /* if (!error) {
                    ...
                    console.log('File added, filesInProcess:', filesInProcess);
                } */
                if (!error) {
                    // totalUploadedSize += file.fileSize;
                    filesInProcess++;
                    // console.log(`Added: ${file.filename}, Total uploaded: ${formatSizeUnit(totalUploadedSize / 1024 / 1024)}`);
                    // console.log(`Added: ${file.filename}, Total uploaded: ${formatSizeUnit(totalUploadedSize)}`);
                    console.log(`Added: ${file.filename}, Total uploaded: ${formatSizeUnit(getTotalUploadedSize())}`);
                    uploadFileBtn.prop('disabled', true);
                }
                ...
                console.log('File processed, filesInProcess:', filesInProcess);
                if (filesInProcess === 0) {
                    toggleUploadButton();
                }
            }, server: {
                    ...
                    if (isVideo) {
                        ...
                        return {
                            abort: () => {
                                ...
                                // Clean up tracking and update state
                                delete pythonApiUploads[fileId];
                                filesInProcess = Math.max(0, filesInProcess - 1);
                                validFiles = validateAllFiles();
                                toggleUploadButton();
                                abort();
                            }
                        ...
                        return {
                            abort: () => {
                                request.abort();
                                // Update state when non-video file is aborted
                                filesInProcess = Math.max(0, filesInProcess - 1);
                                validFiles = validateAllFiles();
                                toggleUploadButton();
                                abort();
                ...
                revert: (filename, load) => {
                    ...
                    if (videoUploadId) {
                        ...
                        const videoRevertURL = `${videoUploadURL}delete`;
                    ...
                    // Ensure proper state management after revert
                    filesInProcess = Math.max(0, filesInProcess - 1);
                    validFiles = validateAllFiles();
                    toggleUploadButton();

                    if (typeof uploadDir !== 'undefined') {
                        // For single upload contexts, enable button if no files are processing
                        if (filesInProcess === 0) {
                ...
                const maxSize = fileTypeMaxSize[fileType]; // size in MB
                const fileSizeMB = file.fileSize / 1024 / 1024;

                // Individual file type size check
                if (fileSizeMB > maxSize) {
                    show_notify(`The file size of ${fileName} exceeds the allowed limit of ${formatSizeUnit(maxSize * 1024 * 1024)}.`, 'fail');
                    return false;
                }

                const currentTotal = getTotalUploadedSize(); // ← Dynamic
                const limitBytes = totalUploadedLimitSize;  // In bytes

                if (currentTotal + file.fileSize > limitBytes) {
                    const remaining = limitBytes - currentTotal;
                    // const readableRemaining = formatSizeUnit(remaining);
                    const readableRemaining = remaining > 0 ? formatSizeUnit(remaining) : '0 MB';
                    console.log('currentTotal:', currentTotal, 'limitBytes:', limitBytes, 'remaining:', remaining);
                    show_notify(`Not enough storage remaining. You can only upload up to ${readableRemaining}.`, 'fail');
                    return false;
                }
        ...
        pond.on('removefile', (error, file) => {
            console.log('File removed, status:', file.status, 'filesInProcess before:', filesInProcess);

            console.log(`Removed: ${file.filename}, Total uploaded: ${formatSizeUnit(getTotalUploadedSize())}`);

            // Handle different file removal scenarios
            if (file.status === FilePond.FileStatus.PROCESSING ||
                file.status === FilePond.FileStatus.PROCESSING_QUEUED) {
                // File was cancelled during upload (scenario 1)
                filesInProcess = Math.max(0, filesInProcess - 1);
            } else if (file.status === FilePond.FileStatus.IDLE ||
                        file.status === FilePond.FileStatus.LOADING) {
                // File was removed before processing started
                filesInProcess = Math.max(0, filesInProcess - 1);
            }

            console.log('File removed, filesInProcess after:', filesInProcess);
        ...
        pond.on('addfile', (error, file) => {
            // This event is fired when a file is added but before onaddfile callback
            console.log('addfile event - file status:', file.status);
        });

        pond.on('processfileabort', (file) => {
            // Additional handler for when file processing is aborted
            console.log('File processing aborted, filesInProcess before:', filesInProcess);
            filesInProcess = Math.max(0, filesInProcess - 1);
            validFiles = validateAllFiles();
            toggleUploadButton();
            console.log('File processing aborted, filesInProcess after:', filesInProcess);
        });
        ...
        pond.on('error', (error,file) => {
            ...
            if (file && file.fileSize) {
                totalUploadedSize = Math.max(0, totalUploadedSize - file.fileSize);
                console.log(`Error: ${file.filename} removed from count, Total now: ${formatSizeUnit(totalUploadedSize)}`);
            }
            validFiles = false;
            toggleUploadButton();
            $('.file-error').text(error.body).show();
    ...
    // Function to get the total uploaded size of all files in the pond
    function getTotalUploadedSize() {
        return pond.getFiles().reduce((total, file) => total + file.fileSize || 0, 0);
    }
    ...
    function validateAllFiles() {
        ...
        if (pondFiles.length === 0) {
            console.log('No files in pond, resetting filesInProcess to 0');
            filesInProcess = 0;
            return false;
        }

        // Count currently processing files
        let actualProcessingFiles = 0;
        let idleFiles = 0;

        pondFiles.forEach(function (fileItem) {
            console.log('File status:', fileItem.status);
            if (fileItem.status === FilePond.FileStatus.PROCESSING ||
                fileItem.status === FilePond.FileStatus.PROCESSING_QUEUED) {
                actualProcessingFiles++;
                allValid = false;
            } else if (fileItem.status === FilePond.FileStatus.IDLE ||
                        fileItem.status === FilePond.FileStatus.LOADING) {
                idleFiles++;
                allValid = false;
            } else if (fileItem.status !== FilePond.FileStatus.PROCESSING_COMPLETE) {
                allValid = false;
            }
        });

        // Update filesInProcess to include both processing and idle files
        filesInProcess = actualProcessingFiles + idleFiles;

        console.log('validateAllFiles - actualProcessingFiles:', actualProcessingFiles, 'idleFiles:', idleFiles, 'totalFilesInProcess:', filesInProcess);
    ...
    function toggleUploadButton() {
        ...
        console.log('Toggle button - validFiles:', validFiles, 'validDirectory:', validDirectory, 'filesInProcess:', filesInProcess);
    ...
    /* function formatSizeUnit(sizeInMB) {
        if (sizeInMB >= 1024) {
    ...
    $(document).on('click', '.cancel-upload', function (e) {
        ...
        // Get all video upload IDs
        const videoUploadIds = Object.values(pythonApiUploads)
        ...
        if (videoUploadIds.length > 0) {
            ...
            // Call Python API to delete videos
            fetch(`${RemoveTmpUploadVideoURL}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    upload_ids: videoUploadIds
                })
            })
                .then(response => response.json())
                .then(data => {
                    // Clear all tracking
                    Object.keys(pythonApiUploads).forEach(key => {
                        delete pythonApiUploads[key];
                    });
                    ...
                    hide_loader();
                    show_notify('Upload cancelled successfully', 'success');

                    // Redirect to index page
                    setTimeout(() => {
                        window.location.href = `${indexURL}`; // Make sure indexURL is defined in your view
                    }, 1000);
                })
                .catch(error => {
                    hide_loader();
                    console.error('Error cancelling uploads:', error);
                    show_notify('Error cancelling uploads', 'fail');
                });
        } else {
            // If no video uploads, just redirect
